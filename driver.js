...
var p_data={mintemp:0,opttemp:0,maxtemp:0,humidity:0,sunrise:0,sunset:0,sunpresence:0,sunnow:0};
var p_timings={time:0,offset:0,dsave:0};
var p_data_formatted={sunset:"",sunrise:""};
	var curr_unix = (new Date).getTime();
		p_timings.time=curr_unix/1000;
...
$.get('http://api.openweathermap.org/data/2.5/weather?q='+eplace+'&APPID=83079073594bc6cbf8e690f28079079e',function(data,success)
{
...
function grab(place)
{
	...
	else
	{
		...
		$.get('http://api.openweathermap.org/data/2.5/weather?APPID=83079073594bc6cbf8e690f28079079e&q='+place,function(data,success)
	{
		var js = data;
...
document.getElementById('desc').innerHTML='<img src = "http://openweathermap.org/img/w/'+js.weather[0].icon+'.png"><br>'+capitalise(js.weather[0].description);
		...
		avgtemp = Math.round(tempformat[1]*(9/5)-459.67);
		...
		p_data.sunrise=parseInt(js.sys.sunrise);
		p_data.sunset=parseInt(js.sys.sunset);
		document.getElementById('daystatus').style.backgroundSize='cover';
		document.getElementById('daystatus').style.backgroundImage='url(https://maps.googleapis.com/maps/api/staticmap?center='+lat+','+lon+'&size=1100x180&maptype=satellite&zoom=14&scale=2)';
document.getElementsByTagName('meta')[7].content='http://w-onebeat.rhcloud.com?place='+place;
document.getElementById('fbshare').onclick=function(){popupwindow('https://www.facebook.com/sharer/sharer.php?u=http://w-onebeat.rhcloud.com?place='+place+'&display=popup&ref=plugin&src=share_button');};
document.getElementsByTagName('title')[0].innerHTML="Weather of '"+place+"'";
...
$.get('https://maps.googleapis.com/maps/api/timezone/json?location='+lat+','+lng+'&timestamp='+tstamp+'&key=AIzaSyDA742kfYz39JZSX2VZ-af1h7mdtBAS-zs',function(data,success)
	{
		...
		p_timings.dsave=dsave;
		p_timings.offset=off+dsave;
		updater(0,real);
		sunstatus();
		sundraw();
...
{
	time = window.setInterval(function(){unix2utc(unix+t);p_timings.time=unix+t;t++;},1000);
}
...
function forecast(lat,lon)
{
	...
	lat=Math.round(lat);
	lon=Math.round(lon);
	var url ='http://api.openweathermap.org/data/2.5/forecast?APPID=83079073594bc6cbf8e690f28079079e&lat='+lat+'&lon='+lon;
...
function grab_geo(lat,long)
{
	...
	{
		document.getElementById('loading').innerHTML='Please wait, gathering current weather data...<img src = "images/loadup.gif" style="width:14px; height:14px;"/>';
		$.getJSON("http://api.openweathermap.org/data/2.5/weather?APPID=83079073594bc6cbf8e690f28079079e&lat="+lat+"&lon="+long,function(data)
	{
	var js = data;
	flushtable();
	puttable();
...
{
	var sson="";
	if(place_date>=1&&place_month>=3&&place_month<=5&&place_date<=31)
	sson="Spring";
	if(place_date>=1&&place_month>=6&&place_month<=8&&place_date<=31)
	sson="Summer";
	if(place_date>=1&&place_month>=9&&place_month<=11&&place_date<=30)
	sson="Autumn";
	if(place_date>=1&&place_month>=12&&place_month<=2&&place_date<=29)
	sson="Winter";
	return sson;

}

function googleplace(ptitle)
{
	var uurl = 'json_google_places.php?place='+ptitle;
$.getJSON(uurl,function(data,success)
{
	var info_add = data.add;
	var info_lat = data.lat;
	var info_lng = data.lng;
});
}
function sunstatus()
{
	...
	p_data_formatted.sunset=convert_unix(p_data.sunset);
	p_data.sunpresence=(p_data.sunset)-(p_data.sunrise);
	var curr_unix = (new Date).getTime();
		curr_unix=curr_unix/1000;
		p_data.sunnow=p_data.sunset-p_timings.time;
}
